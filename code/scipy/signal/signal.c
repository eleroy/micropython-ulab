...
#if ULAB_SCIPY_SIGNAL_HAS_OACONVOLVE_ENVELOP

mp_obj_t signal_oaconvolve_envelop(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    static const mp_arg_t allowed_args[] = {
        {MP_QSTR_a, MP_ARG_REQUIRED | MP_ARG_OBJ, {.u_rom_obj = MP_ROM_NONE}},
        {MP_QSTR_v, MP_ARG_REQUIRED | MP_ARG_OBJ, {.u_rom_obj = MP_ROM_NONE}},
    };
...
// deal with linear arrays only
    ...
    for (size_t i = 0; i < signal_len; i += segment_len) {
        ...
        for (size_t j = 0; j < fft_size; j++) {
            ...
            real = (segment_fft_array[j * 2] * kernel_fft_array[j * 2] - segment_fft_array[j * 2 + 1] * kernel_fft_array[j * 2 + 1])*2;
            imag = (segment_fft_array[j * 2] * kernel_fft_array[j * 2 + 1] + segment_fft_array[j * 2 + 1] * kernel_fft_array[j * 2])*2;
        }else{
            real = 0;
            imag = 0;
        }
...
MP_DEFINE_CONST_FUN_OBJ_KW(signal_oaconvolve_envelop_obj, 2, signal_oaconvolve_envelop);
#endif /* ULAB_SCIPY_SIGNAL_HAS_OACONVOLVE_ENVELOP */

#if ULAB_SCIPY_SIGNAL_HAS_HILBERT

mp_obj_t signal_hilbert(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    static const mp_arg_t allowed_args[] = {
        {MP_QSTR_a, MP_ARG_REQUIRED | MP_ARG_OBJ, {.u_rom_obj = MP_ROM_NONE}},
    };
...
// deal with linear arrays only
    ...
    int high_pass_freq = (fft_size*90000)/1000000;//90kHZ high pass
...
// deal with linear arrays only
    ...
    if (bp_in->items[0] == mp_const_none) {
        bp_start = 1; // Valeur par défaut pour item1
    } else if (!mp_obj_is_int(bp_in->items[0]) && !mp_obj_is_float(bp_in->items[0])) {
        mp_raise_TypeError(MP_ERROR_TEXT("First item must be int, float, or None"));
    }else{
        bp_start = mp_obj_get_int(bp_in->items[0]);
    }      
...
static const mp_rom_map_elem_t ulab_scipy_signal_globals_table[] = {
    ...
    #endif
    #if ULAB_SCIPY_SIGNAL_HAS_OACONVOLVE_ENVELOP
        { MP_ROM_QSTR(MP_QSTR_oaconvolve_envelop), MP_ROM_PTR(&signal_oaconvolve_envelop_obj) },
    #endif
    #if ULAB_SCIPY_SIGNAL_HAS_HILBERT
